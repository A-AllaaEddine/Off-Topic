...
const Vote = () => {
  ...
  const [newPlayersArray, setNewPlayersArray] = useState(
    playersArray.filter((Player) => Player.id !== votingPlayer.id)
  );
  ...
  const playerVoteHandler = (votedPlayer) => {
    ...
    if (i < playersArray.length) {
      setNewPlayersArray(
        playersArray.filter((Player) => Player.id !== playersArray[i].id)
      );
            ...
            {newPlayersArray &&
              newPlayersArray.map((votedPlayer) => {
                return (
                  <div
                    key={votedPlayer.id}
                    onClick={() => playerVoteHandler(votedPlayer)}
                  >
            ...
            {topics &&
              topics.map((topic) => {
                return (
                  <div
                    key={topic.id}
                    onClick={() => imposterVoteHandler(topic)}
                    className="voting-topics"
                  >
                    <h2 onClick={() => imposterVoteHandler(topic)}>
                      {topic.name}
                    </h2>
                  </div>
                );
